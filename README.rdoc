...
This library also contains a Ruby client library to the Engine Yard Cloud API.

Setup:

    token = EY::CloudClient.authenticate("your@email.com", "password")
    client = EY::CloudClient.new(token)

Apps:

    apps = client.apps # loads all your app data at once; caches result
...
Environments:
    ...
    envs = client.environments # loads all your environment data at once; caches result
...
Valid `app_server_stack_name` values: `nginx_unicorn, nginx_passenger3, nginx_nodejs, nginx_thin, nginx_puma`. Your account may require an early access feature to be enabled.
